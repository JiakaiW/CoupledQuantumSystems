...
max_ql = 2
max_ol = 3
t_tot = 20 # Made float for linspace
amp_val = 0.4/(2*np.pi) # Renamed from amp to amp_val to avoid conflict with pss.amp
...
drive_term_sim = DriveTermSymbolic(
    ...
    amplitude=amp_val/(2*np.pi),                       # Overall amplitude for the envelope
...
qutip_pulse_args = {
    'amp': amp_val, # Note: old envelope might have 2*pi scaling, new pure shape does not.
